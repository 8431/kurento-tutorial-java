...
public class CallMediaPipeline {
  ...
  public static final String RECORDING_PATH = "file:///u/home/media/";
  public static final String DATA_PATH = "/u/home/media/";
  ...
  public CallMediaPipeline(KurentoClient kurento, String from, String to) {
    ...
    String currrentDate = sd.format(new Date());
    File file = new File(DATA_PATH +currrentDate);
    if(!file.exists()){
    	if(!file.mkdirs()){
    		System.out.println("Failure to create the directory where the target file is located!");
    	}
    }
    currrentDate += "/" + df.format(new Date())+ "-";
    
    recorderCaller = new RecorderEndpoint.Builder(pipeline, RECORDING_PATH + currrentDate + from + RECORDING_EXT)
        .build();
    recorderCallee = new RecorderEndpoint.Builder(pipeline, RECORDING_PATH + currrentDate + to + RECORDING_EXT)
        .build();
