...
public class CallHandler extends TextWebSocketHandler {
  ...
  public void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
    ...
    switch (jsonMessage.get("id").getAsString()) {
      ...
      case "call":
        ...
        break;
      case "webRTCForSend":
		  webRTCForSend(user, jsonMessage);
		  break;
  ...
  private void register(WebSocketSession session, JsonObject jsonMessage) throws IOException {
    ...
    if (name.isEmpty()) {
      ...
      responseMsg = "rejected: user '" + name + "' already registered replace!!!!!!";
	...
	response1.addProperty("id", "appurl");
	response1.addProperty("appurl", props.getProperty("DNS_NAME"));
	
	caller.sendMessage(response1);
  ...
  private void webRTCForSend(UserSession caller, JsonObject jsonMessage) throws IOException {
    String to = jsonMessage.get("to").getAsString();
    String from = jsonMessage.get("from").getAsString();
    JsonObject response = new JsonObject();
    ...
    if (registry.exists(to)) {
      ...
      response.addProperty("id", "webRTCForSend");
      response.addProperty("from", from);
      response.addProperty("content", content);
